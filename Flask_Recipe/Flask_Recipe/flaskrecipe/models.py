...
class Item(db.Model):
    ...
    #recipe = db.relationship('Recipe_Ingredients', backref='recipe_item', lazy=True, uselist=False) #one to one with Recipe_Ingredients

    # One item belongs to one list
    #list = db.relationship('List_Items', backref='list_item', lazy=True, uselist=False)
    def __repr__(self):
